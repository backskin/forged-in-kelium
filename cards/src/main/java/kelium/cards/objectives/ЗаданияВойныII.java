...
public final class ЗаданияВойныII {
    ...
    public static final class Блицкриг extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Блицкриг", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                Награда.монеты(5), Награда.модуль("attack").иКартыАрсенала(1),
                "СВОБОДНЫЙ БОЙ",
                "В ЭТОТ ХОД уничтожь два жетона противника одним своим жетоном "
                + "войска — награда 5 монет. Если это войско — пехота, условие "
                + "усилено: жетон модуля атаки и карта арсенала. Пехота, снявшая "
                + "двоих за ход, стоит целого завода.");
        }
    ...
    public static final class НаЧужомДворе extends ЗаданиеВКоде {
        ...
        @Override
        public boolean burn(CardContext ctx) {
            return свободноеСнаряжение(ctx, 2);
        }
    ...
    public static final class Клещи extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Клещи", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.боеприпасы(3), Награда.модуль("attack").иКартыЗаданий(1),
                "СВОБОДНОЕ ДВИЖЕНИЕ",
                "Имей свои войска на двух гексах, соседних с одним и тем же гексом, "
                + "где есть войска противника, — награда 3 боеприпаса. Замкни клещи "
                + "с трёх сторон, и условие усилено: жетон модуля атаки и карта "
                + "задания. Клещи не бьют — они отнимают выход.");
        }
    ...
    public static final class ПустойДвор extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Пустой двор", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.монеты(4), Награда.обломки(2).иКартуСВитрины(),
                "СНАРЯЖЕНИЕ ДВУМЯ ЗДАНИЯМИ",
                "Имей три своих войска вне гексов, где стоят твои здания, и ни "
                + "одного своего войска на этих гексах, — награда 4 монеты. Если все "
                + "три стоят на разных гексах, условие усилено: 2 обломка и карта "
                + "арсенала на выбор из открытых. Двор пустеет ровно тогда, когда "
    ...
    public static final class ГосподствоВНебе extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Господство в небе", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.монеты(4), Награда.модуль("attack").иКартыАрсенала(1),
                "+1 К СКОРОСТИ ОДНОГО РОДА ДО КОНЦА ХОДА",
                "Имей свою авиацию на гексе, где есть жетоны противника, — награда "
                + "4 монеты. Если на этом гексе стоит ЦУ противника, условие "
                + "усилено: жетон модуля атаки и карта арсенала. В воздушную ячейку "
                + "чужого гекса влезает ровно одна авиация — чья первая, того и небо.");
        }
    ...
    public static final class ОтветныйУдар extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Ответный удар", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                Награда.боеприпасы(3), Награда.модуль("attack").иКартуСВитрины(),
                "2 боеприпаса",
                "В ЭТОТ ХОД разыграй карту, если после начала твоего прошлого хода "
                ...
                + "боях ты не потерял ни одного своего жетона, условие усилено: "
                + "жетон модуля атаки и карта арсенала на выбор из открытых. Ответный бой случается не в "
    ...
    public static final class Разорение extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Разорение", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                Награда.монеты(3), Награда.обломки(3).иКартыАрсенала(1),
                "2 монеты",
                "В ЭТОТ ХОД уничтожь добытчик противника — награда 3 монеты. Если "
                + "этот добытчик был запитан, условие усилено: 3 обломка и карта "
                + "арсенала. Отнятый добытчик стоит сопернику всех оставшихся "
                + "раундов добычи.");
        }
    ...
    public static final class ОхотаНаСильного extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Охота на сильного", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                + "жетон был зданием, условие усилено: жетон модуля атаки и карта "
                + "задания. Догоняющему выгодно бить того, кто оторвался, — и это "
    ...
    public static final class ТрофейныйОбоз extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Трофейный обоз", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.монеты(3), Награда.обломки(2).иКартуСВитрины(),
                "СВОБОДНАЯ НАУКА",
                "Имей в трофеях жетоны трёх разных видов — пехота, техника, "
                + "авиация, вышка и каждый тип здания считаются отдельно. Награда: "
                + "3 монеты. Собери четыре разных вида, и условие усилено: 2 "
                + "обломка и карта арсенала на выбор из открытых. Обоз показывает не силу удара, а широту "
                + "фронта.");
        }
