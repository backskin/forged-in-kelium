    ...
    public static final class РазведкаНедр extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Разведка недр", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                Утиль.РАЗВЁРТЫВАНИЕ,
                "В ЭТОТ ХОД забери 2 контейнера (добытчиками или войсками) — "
                + "награда бесплатный Рынок. Усиления у карты нет: два контейнера за "
                + "один ход и так работа всей смены. Каждый добытчик в Добыче "
                + "выбирает одно: келемий или контейнер.");
        }

        // ТРИ КОНТЕЙНЕРА ЗА ХОД БЫЛИ НЕВОЗМОЖНЫ (замер 19.09.2026). Стенд
        ...
        @Override
        public boolean satisfied(CardContext ctx) {
            return ход(ctx).containersTaken >= 2;
        }
