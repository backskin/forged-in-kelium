...
public final class НачальныеЗадания {
    ...
    public static final class Патроны extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("Патроны", СОСТОЯНИЕ,
                "Имей не менее трёх боеприпасов в хранилище",
                "Имей не менее трёх боеприпасов в хранилище — награда трофей и карта "
    ...
    public static final class ПерваяСделка extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("Первая сделка", ПРОИСШЕСТВИЕ,
                "В ЭТОТ ХОД в действие Рынок воспользуйся предложением карты на рынке",
                "В ЭТОТ ХОД в действие Рынок воспользуйся предложением карты на рынке "
                + "— награда трофей и карта задания. Напечатанный обмен планшета не "
                + "подходит: нужно предложение с открытой карты рынка.");
        }
    ...
    public static final class Коммутация extends Начальная {
        ...
        @Override
        public Лицо лицо() {
            // ПЕЧАТНАЯ КАРТА № 05 (25.09.2026): здание — ровно в двух гексах от
            // своего ЦУ (прежде годился любой гекс, кроме гекса ЦУ).
            return лицоНачальной("Коммутация", СОСТОЯНИЕ,
                "Имей на поле запитанное здание на расстоянии в 2 гекса от твоего ЦУ",
                "Имей на поле запитанное здание на расстоянии в 2 гекса от твоего ЦУ "
                + "— награда трофей и карта задания. Энергия туда сама не дотянется: "
                + "её надо развести Питанием.");
        }

        /** Своё здание ровно в двух гексах от ближайшего своего ЦУ. */
        private static boolean вДвухГексах(CardContext ctx, BuildingToken b) {
            return гексовДо(ctx, b.hexId, гексыЦУ(ctx.me())) == 2;
        }

        @Override
        public boolean satisfied(CardContext ctx) {
            for (BuildingToken b : ctx.me().buildingsOnField()) {
                if (b.powered() && вДвухГексах(ctx, b)) {
                    return true;
                }
            }
            return false;
        ...
        @Override
        public double progress(CardContext ctx) {
            // ЗДЕСЬ БЛИЗОСТЬ НАСТОЯЩАЯ: требование распадается на два состояния
            // поля. Здание в двух гексах уже стоит — половина; осталось развести
            // на него энергию Питанием.
            boolean естьТам = false;
            for (BuildingToken b : ctx.me().buildingsOnField()) {
                if (вДвухГексах(ctx, b)) {
                    if (b.powered()) {
                        return 1.0;
                    }
                    естьТам = true;
            ...
            return естьТам ? 0.5 : 0.0;
        }

        @Override
        public String needed(CardContext ctx) {
            return satisfied(ctx) ? "" : "запитать здание в двух гексах от своего ЦУ";
        }
    ...
    public static final class Выход extends Начальная {
        ...
        @Override
        public Лицо лицо() {
            // ПЕЧАТНАЯ КАРТА № 06 (25.09.2026): «на расстоянии в 2 гекса» —
            // ровно два гекса по полю, приписки «и больше» на рисунке нет.
            return лицоНачальной("Выход", СОСТОЯНИЕ,
                "Имей на поле войска на расстоянии в 2 гекса от твоего ЦУ",
                "Имей на поле войска на расстоянии в 2 гекса от твоего ЦУ — награда "
                + "трофей и карта задания. Вышка неподвижна, так что выйти придётся "
                + "кем-то живым.");
        }
        ...
        @Override
        public String needed(CardContext ctx) {
            return satisfied(ctx) ? "" : "поставить войско ровно в двух гексах от своего ЦУ";
        }
    ...
    public static final class Жила extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("Жила", СОСТОЯНИЕ,
                "Имей не меньше трёх келемия в хранилище",
                "Имей не меньше трёх келемия в хранилище — награда трофей и карта задания. "
    ...
    public static final class Находка extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("Находка", СОСТОЯНИЕ,
                "Имей у себя в запасе хотя бы 2 неоткрытых контейнера",
                "Имей у себя в запасе хотя бы 2 неоткрытых контейнера — награда "
    ...
    public static final class ПервыйТрофей extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("Первый трофей", СОСТОЯНИЕ,
                "Имей у себя на свалке хотя бы один жетон",
                "Имей у себя на свалке хотя бы один жетон — награда трофей и карта "
    ...
    public static final class ВУнисон extends Начальная {
        ...
        public Лицо лицо() {
            return лицоНачальной("В унисон", ПРОИСШЕСТВИЕ,
                "В ЭТОТ ХОД сыграй тот же основной приказ, что и другой игрок",
                "В ЭТОТ ХОД сыграй тот же основной приказ, что и другой игрок, — "
