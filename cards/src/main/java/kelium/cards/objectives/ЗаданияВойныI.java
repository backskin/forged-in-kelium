    ...
    public static final class ШтабНаПередовой extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Штаб на передовой", ОБЫЧНАЯ, СОСТОЯНИЕ,
                "Имей своё военное здание на расстоянии не больше 2 гексов от "
                + "гекса с ЦУ противника",
                "Это здание примыкает к гексу с ЦУ противника",
                Награда.монеты(3), Награда.трофеи(2),
                Утиль.СНАРЯЖЕНИЕ,
                "Имей своё военное здание — казарму, завод или авиабазу — на "
                + "расстоянии не больше 2 гексов от гекса с ЦУ противника, — "
                + "награда 3 монеты. Если это здание примыкает к гексу с чужим ЦУ, "
                + "условие усилено: 2 трофея. Производство под самым носом врага "
                + "экономит каждое перемещение — и первым попадает под удар.");
        }
        ...
        /** Наименьшее расстояние от своего военного здания до чужого ЦУ. */
        ...
        public double progress(CardContext ctx) {
            ...
            // буквально есть близость на поле. Порог требования 2 гекса; отсчёт
            // от шести, дальше разница для карты неразличима.
            Integer d = ближайшее(ctx);
            if (d == null) {
                return 0.0;
            }
            ...
            return доля(Math.max(0, 6 - d), 6 - 2) * 0.6;
        }
        ...
        public String needed(CardContext ctx) {
            ...
            return d == null ? "построить военное здание"
                : "построить военное здание на " + (d - 2) + " гекс(а) ближе к чужому ЦУ";
        }
    ...
    public static final class Осада extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Осада", ОБЫЧНАЯ, СОСТОЯНИЕ,
                "Имей среди уничтоженных жетонов здание противника",
                "Среди твоих трофеев есть добытчик противника",
                Награда.монеты(1), Награда.модуль("attack"),
                Утиль.СНАРЯЖЕНИЕ,
                "Имей среди уничтоженных жетонов здание противника — награда 1 монета. Если "
