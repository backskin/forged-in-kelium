    ...
    public static final class Отгрузка extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Отгрузка", ОБЫЧНАЯ, ЖЕРТВА,
                ...
                "Сдай четыре контейнера",
                Награда.монеты(3), Награда.трофеи(2),
                Утиль.БОЕПРИПАС_В_НАСТУПЛЕНИЕ,
                "Сдай два своих неоткрытых контейнера — награда 3 монеты. Сдай "
                + "четыре, и условие усилено: 2 трофея. Контейнер стоит ровно "
                + "столько, сколько в нём лежит, а лежит в нём неизвестно что.");
        }
        ...
        protected Map<String, Object> усиленнаяЖертваВЗаписи() {
            return Map.of("predicate", "sacrifice_enhanced",
                "params", Map.of("resource", "container", "amount", 4));
        }
    ...
    public static final class Разоружение extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Разоружение", ОБЫЧНАЯ, ЖЕРТВА,
                "Верни в свой запас два своих войска с поля",
                "Это техника и/или авиация, и оба жетона с одного гекса",
                Награда.боеприпасы(3), Награда.модуль("assembly"),
                Утиль.ЭНЕРГИЯ_ИЛИ_МОДУЛИ,
                "Верни в свой запас два своих войска с поля — награда 3 боеприпаса. "
                + "Если это техника и/или авиация и оба жетона с одного гекса, "
                + "условие усилено: жетон модуля сборки. Сданные войска не "
                + "считаются уничтоженными и трофеями противнику не идут.");
        }
    ...
    public static final class Зачистка extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Зачистка", ОБЫЧНАЯ, ЖЕРТВА,
                "Имей среди уничтоженных жетонов здание противника и верни его владельцу",
                "Верни владельцу ещё один уничтоженный жетон",
                Награда.монеты(3).иБоеприпасы(2), Награда.модуль("assembly"),
                Утиль.НАУКА,
                "Имей среди уничтоженных жетонов здание противника и верни его владельцу — награда 3 "
                + "монеты и 2 боеприпаса. Верни владельцу ещё один уничтоженный жетон, и "
                + "условие усилено: жетон модуля сборки. Отданный уничтоженный жетон очков не приносит: "
                + "за это и платят.");
        }
        ...
        /** Требуется хотя бы одно чужое ЗДАНИЕ среди уничтоженных жетонов, а не любой жетон. */
        @Override
        public boolean satisfied(CardContext ctx) {
            for (var t : ctx.me().destroyedTokens) {
        ...
        @Override
        public boolean satisfiedEnhanced(CardContext ctx) {
            return ctx.me().destroyedTokens.size() >= 2;
        }
        ...
        public double progress(CardContext ctx) {
            ...
            // Нужно именно чужое ЗДАНИЕ среди уничтоженных жетонов. Трофейные ВОЙСКА к цели не
        ...
        @Override
        public String needed(CardContext ctx) {
            return satisfied(ctx) ? "" : "взять здание противника на место уничтоженных жетонов";
        }
    ...
    public static final class ЛинияФронта extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Линия фронта", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                + "усилено: 3 трофея. Вышка стоит даром и не "
    ...
    public static final class ПромышленныйУзел extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Промышленный узел", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.монеты(3), Награда.трофеи(3),
                Утиль.СТРОЙКА,
                "Твои здания образуют непрерывное соседство, связывающее три гекса, "
                + "лежащих по прямой, — награда 3 монеты. Если в этом соседстве "
                + "участвуют две твои авиабазы, условие усилено: 3 трофея. Авиабаза просит три кубика энергии, и две подряд — это "
                + "уже вся сеть.");
        }
