    ...
    public static final class Биржа extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Биржа", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                "В ЭТОТ ХОД воспользуйся двумя разными предложениями Рынка. "
                + "Награда: 3 монеты. Напечатанные курсы планшета и предложения карты "
                + "рынка считаются по отдельности. Усиления у карты нет.");
        }
    ...
    public static final class НаучныйОтдел extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Научный отдел", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                "В ЭТОТ ХОД воспользуйся двумя разными предложениями Науки",
                null,
                Награда.действием("market"), Награда.нет(),
                Утиль.РАЗВЁРТЫВАНИЕ,
                "В ЭТОТ ХОД воспользуйся двумя разными предложениями Науки — "
                + "награда бесплатный Рынок. Шаг по треку и каждый обмен трофеев "
                + "на планшете технологий считаются отдельным предложением. "
        ...
        @Override
        public boolean satisfied(CardContext ctx) {
            return ход(ctx).scienceOffersUsed.size() >= 2;
        }
        ...
        public double progress(CardContext ctx) {
            ...
            return ступени(доля(взято, 2),
                готовность(доля(трофеи, 2)));
        }
        ...
        public String needed(CardContext ctx) {
            ...
            return ctx.have(kelium.core.Resource.TROPHY) < 2
                ? "накопить трофеи на два предложения планшета технологий"
                : "взять ещё " + (2 - есть) + " предложение планшета технологий";
        }
    ...
    public static final class НаучныйРывок extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Научный рывок", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                + "бесплатное Снаряжение. Стой на втором шаге сразу на двух треках, и "
                + "условие усилено: 1 монета. Второй шаг — первое место, где трек "
                + "начинает платить очками.");
        }
    ...
    public static final class ВаБанк extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Ва-банк", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.монеты(1),
                Утиль.БОЕПРИПАС_В_НАСТУПЛЕНИЕ,
                "Имей одновременно ноль монет и ноль боеприпасов — награда "
                + "бесплатная Добыча. Не имей даже келемий, и условие усилено: 1 монета. Ва-банк играют только на голом столе.");
        }
