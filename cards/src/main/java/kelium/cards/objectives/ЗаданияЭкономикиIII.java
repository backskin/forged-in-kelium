...
public final class ЗаданияЭкономикиIII {
    ...
    /** o33 «Биржа» — два разных предложения планшета рынка за ход. */
    public static final class Биржа extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Биржа", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                "В ЭТОТ ХОД воспользуйся двумя разными предложениями Рынка",
                null,
                Награда.действием("assembly"), Награда.нет(),
                Утиль.БОЕПРИПАС_В_НАСТУПЛЕНИЕ,
                "В ЭТОТ ХОД воспользуйся двумя разными предложениями Рынка. Награда: "
                + "3 монеты. Напечатанные курсы планшета и предложения карты рынка "
                + "считаются по отдельности. Усиления у карты нет.");
        }

        @Override
        public boolean satisfied(CardContext ctx) {
            return ход(ctx).marketOffersUsed.size() >= 2;
        }
        ...
        public double progress(CardContext ctx) {
            return доля(ход(ctx).marketOffersUsed.size(), 2);
        }

        @Override
        public String needed(CardContext ctx) {
            int есть = ход(ctx).marketOffersUsed.size();
            return есть >= 2 ? "" : "воспользоваться ещё " + (2 - есть)
                + " предложением Рынка";
        }
    ...
    /** o34 «Научный отдел» — три разных предложения Науки за ход. */
    public static final class НаучныйОтдел extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Научный отдел", ОБЫЧНАЯ, ПРОИСШЕСТВИЕ,
                ...
                null,
                Награда.действием("market"), Награда.нет(),
                Утиль.РАЗВЁРТЫВАНИЕ,
                "В ЭТОТ ХОД воспользуйся тремя разными предложениями Науки. Награда: "
                + "1 монета и 2 боеприпаса. Шаг по треку и каждый обмен трофеев на "
                + "планшете технологий считаются отдельным предложением; обменов на "
                + "планшете три, значит три предложения — это ровно все они за один "
                + "ход. Усиления у карты нет.");
        }
        ...
        public double progress(CardContext ctx) {
            ...
            int взято = ход(ctx).scienceOffersUsed.size();
            int трофеи = ctx.have(kelium.core.Resource.TROPHY);
            return ступени(доля(взято, 3),
        ...
        public String needed(CardContext ctx) {
            ...
            return ctx.have(kelium.core.Resource.TROPHY) < 4
                ? "накопить трофеи на три предложения планшета технологий"
                : "взять ещё " + (3 - есть) + " предложение планшета технологий";
        }
    ...
    public static final class НаучныйРывок extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Научный рывок", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                Награда.действием("assembly"), Награда.картаАрсенала(),
                Утиль.РАЗВЁРТЫВАНИЕ,
                "Дойди до второго шага хотя бы на одном треке технологий — награда бесплатное Снаряжение"
                + "монеты. Стой на втором шаге сразу на двух треках, и условие усилено: 1 "
                + "карта арсенала. Второй шаг — первое место, где трек начинает платить "
                + "очками.");
        }
    ...
    public static final class ВаБанк extends ЗаданиеВКоде {
        ...
        public Лицо лицо() {
            return new Лицо("Ва-банк", ОБЫЧНАЯ, СОСТОЯНИЕ,
                ...
                "Не имей даже келемий",
                Награда.действием("mining"), Награда.трофеи(1),
                Утиль.БОЕПРИПАС_В_НАСТУПЛЕНИЕ,
                "Имей одновременно ноль монет и ноль боеприпасов — награда бесплатная Добыча"
                + "боеприпас. Не имей даже келемий, и условие усилено: 1 трофей. Ва-банк "
                + "играют только на голом столе.");
        }
