...
public final class ЗаданияЭкономикиII {
    ...
    public static final class Стройбум extends ЗаданиеВКоде {
        ...
        /**
         * ГЕКСЫ, ГДЕ В ЭТОТ ХОД ПОСТРОЕНО ЗДАНИЕ. Прежде считались все
         * строительные операции вместе со сносом; на печатной карте —
         * «построй 2 здания», и снос туда не идёт.
         */
        private java.util.List<String> гексыОпераций(CardContext ctx) {
            return java.util.List.copyOf(new java.util.LinkedHashSet<>(ход(ctx).builtOnHexes));
        }

        @Override
        public Лицо лицо() {
            // ПЕЧАТНАЯ КАРТА «Развёртывание» № 20 (25.09.2026): усиление — два
            // здания на несоседних гексах, трёх больше не нужно.
            return печатное("Стройбум", ПРОИСШЕСТВИЕ,
                "В ЭТОТ ХОД построй 2 здания на разных гексах",
                "На несоседних между собой гексах.",
        ...
        public boolean satisfiedEnhanced(CardContext ctx) {
            return satisfied(ctx) && ctx.chooseNonAdjacent(гексыОпераций(ctx), 2);
        }
