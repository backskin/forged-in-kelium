...
public final class Effects {
    ...
    public static Map<String, Object> apply(String eid, GameState s, int seat, Map<String, Object> params) {
        ...
        return switch (eid) {
            ...
            case "empty" -> Map.of();
            // РЕАКЦИЯ ЧЕРЕЗ ОБЩИЙ РЕЕСТР НЕ ИГРАЕТСЯ: её окно открывает бой, и
            // «сыграть» её вне окна значит отдать карту даром. Пустой ответ
            // здесь и значит «верх не сложился».
            case Реакции.ЭФФЕКТ -> Map.of();
    ...
    public static boolean isImplemented(String eid) {
        ...
        return switch (eid) {
            case "gain", "heal_one", "heal_all_own", "heal_hex", "free_action",
                 ...
                 // Верхи колоды заданий 1.16.0 (09.09.2026): один обмен в Науке
                 // или на Рынке — обычный эффект; «reaction» — не эффект в этом
                 // смысле вовсе, а ОБЪЯВЛЕНИЕ вида реакции: играет её бой,
                 // открывая своё окно (kelium.engine.Реакции). Считать её
                 // нереализованной нельзя — карту тогда молча выбросят из
                 // колоды на подготовке.
                 "exchange_science_or_market", "reaction",
    ...
    static Map<String, Object> freeAction(GameState s, int seat, Map<String, Object> p) {
        ...
        if (res != null) {
            ...
            // ТЕЛЕМЕТРИЯ БЕСПЛАТНОГО ДЕЙСТВИЯ НАРУЖУ (19.09.2026). «Действие
            // состоялось» и «действие принесло пользу» — разные вещи: Бой без
            // единого залпа и Стройка без единой операции формально удаются.
            // Дизайнер спросил ровно про второе, и без этих чисел ответить
            // нечем — из «ran = true» пользы не видно.
            got.put("telemetry", res.telemetry());
        }
        return got;
    }
    ...
    static Map<String, Object> evacuate(GameState s, int seat, Map<String, Object> p) {
        ...
        // 2. КУДА УХОДИМ: ЛЮБОЙ гекс поля, кроме покидаемого.
        //
        // ПРАВИЛО ТЕЛЕПОРТАЦИИ (дизайнер, 06.09.2026): эффект, который жетон не
        // ведёт, а снимает и ставит, правил прохода не соблюдает вовсе —
        // проходить нечего, пути нет. Ограничивает только вместимость гекса, и
        // она проверяется ниже, при постановке каждого жетона.
        //
        // Прежде здесь стояла ЗОНА СТРОЙКИ игрока. Этого ограничения нет ни в
        // тексте карты («перемести их на другой гекс»), ни в правилах: движок
        // придумал его сам, и «Эвакуация» много слабее написанного.
        // Недоступны только гексы, где жетону физически негде стоять: запретные
        // и накрытые грядой зарождения — это состояние картона, а не правило
        // прохода.
        java.util.Set<String> zone = new java.util.LinkedHashSet<>();
        for (kelium.core.Hex h : s.field.hexes.values()) {
