...
public final class Scoring {
    ...
    public static Map<String, Integer> scorePlayer(GameState state, int seat) {
        ...
        // одновременно (trophy_per_vp: 3, trophy_storage_vp_per_unit: 1.0), и
        ...
        // ключа trophy_storage_vp_per_unit ПОЛНОСТЬЮ ЗАМЕНЯЕТ курс trophy_per_vp,
        ...
        int trophyPool = p.resources.trophy() + p.destroyedValue();
        int trophyVp;
        if (econ.containsKey("trophy_storage_vp_per_unit")) {
            trophyVp = (int) Math.floor(trophyPool
                * ((Number) econ.get("trophy_storage_vp_per_unit")).doubleValue());
        } else {
        ...
        // «Военный трек» (эксперимент leftover_destroyed_vp_per): накоплено в Возврат.
        ...
        // ЗВЁЗДЫ НА УСТАНОВЛЕННЫХ КАРТАХ АРСЕНАЛА (арсенал 7.0.0, экспорт
        // дизайнера 13.09.2026). Дизайнер 10.09.2026: «установленные карты
        // арсенала МОГУТ иметь на себе ПО, а могут и не иметь; это нарисовано на
        // карте». Плоское очко за установку снято ещё 14.09.2026 (ключ
        // economy.vp_per_installed_arsenal = 0); здесь очки печатные — поле
        // vp_on_card записи карты, как у супер-арсенала. Сожжённая или удалённая
        // карта звёзд не приносит: считается только то, что лежит установленным.
        int звёзды = 0;
        var arsLib = Ctx.cards(state, "arsenal");
        for (String cid : p.allInstalledArsenal()) {
            var card = arsLib.find(cid);
            if (card != null && card.get("vp_on_card") instanceof Number n) {
                звёзды += n.intValue();
            }
        }
        if (звёзды != 0) {
            breakdown.put("arsenal_stars", звёзды);
        }
        ...
        // economy.trophy_storage_vp_per_unit теперь применяется РОВНО ОДИН РАЗ,
        // выше, в строке "trophy" breakdown'а (либо он, либо старый trophy_per_vp
        // — не оба сразу). Раньше здесь начислялась ЕЩЁ ОДНА, отдельная порция
        // очков по той же паре (trophy + destroyedValue) с тем же курсом —
        // при trophy_per_vp:3 и trophy_storage_vp_per_unit:1.0 (действующий свод)
