...
public final class Arsenal5Abilities {
     ...
     * обломок дают за КАЖДЫЙ удалённый жетон, а не только за чужой; потолка в
     * четыре обломка больше нет; и карта после применения УДАЛЯЕТСЯ ИЗ ИГРЫ.
     *
     ...
     * обломки, когда она уже не нужна. Цена та же: гекс выжжен навсегда, карта
     * ушла из игры.
     */
    private static final class SpecNuclearStrikeBurn implements Ability, OptionSource {
        ...
        @Override public boolean perform(GameState state, int seat, Choice chosen, Agent agent) {
            ...
            // Обломков за них не дают - они ничьи.
            h.neutrals.clear();
            for (int side = 0; side < 6; side++) {
            ...
            if (жетонов > 0) {
                kelium.engine.Storage.addDebrisCapped(state, me, жетонов);
            }
        ...
        @Override public Hint hint() {
            return new Hint(Bottleneck.TROPHY, 3.0, Horizon.NOW, null,
                "выжечь гекс и разобрать всё на обломки, но карта сгорит", true);
        }
    }
