 ...
 * кубик», а после переименования ресурса (2026-08-15, «трофей» →
 * «трофей») это тот же кубик, что теперь И ТАК ограничен складом наравне с
 * келемием/боеприпасом (см. {@code Storage.trophyMax}) — [КОНФЛИКТ], из-за
 * которого карта была отложена, формально снят; осталось решить МЕХАНИКУ карты
 * (гарантированный доп. пул на 3 трофея сверх общего бюджета, или что-то ещё)
...
public final class Arsenal2Abilities {
     ...
     */
    /**
     * «ТРОФЕЙНЫЙ СКЛАД» (b13) — ячейка под ОДИН уничтоженный жетон.
     *
     ...
     *
     * <p>Цена тоже есть, и она честная: задержанный уничтоженный жетон не конвертируется в
     * трофей, а трофеи — источник победных очков.
     */
    private static final class TrophyStorage implements Ability {
        ...
        @Override public java.util.Set<Hook> hooks() {
            return java.util.EnumSet.of(Hook.RETURN_KEEP_DESTROYED);
        }
    ...
    private static final class ExtraTrophyOnSpawnFlip implements Ability {
        ...
        @Override public Hint hint() {
            return new Hint(Bottleneck.TROPHY, 2.0, Horizon.REST_OF_GAME, null,
                "переворот тайла зарождения приносит трофей сверху", false);
        }
    }
}
