...
public final class Arsenal3Abilities {
     ...
     *
     * <p>Прежние «Геологи» давали лишнее трофей за каждый переворот тайла
    ...
    private static final class SpecBoarding implements Ability, OptionSource {
        ...
        @Override public Hint hint() {
            return new Hint(Bottleneck.TROPHY, 2.0, Horizon.THIS_ROUND, null,
                "чужой жетон на место уничтоженных жетонов ценой своей пехоты", false);
        }
    }

    /**
     * «Обмен пленными»: СПЕЦ — верни себе в запас свои жетоны с места уничтоженных жетонов
    ...
    private static final class SpecRansomPrisoners implements Ability, OptionSource {
        ...
        /** Мои жетоны в месте уничтоженных жетонов места {@code holder}. */
        private static List<Token> mine(GameState state, int seat, PlayerState holder) {
            List<Token> out = new ArrayList<>();
            for (Token t : holder.destroyedTokens) {
                if (t.owner() == seat) {
                    out.add(t);
                }
            }
        ...
        @Override public boolean perform(GameState state, int seat, Choice chosen, Agent agent) {
            ...
            for (Token t : prisoners) {
                ...
                holder.destroyedTokens.remove(t);
    ...
    /**
     * «Трофейный сейф»: +2 ячейки склада, но ТОЛЬКО под уничтоженные жетоны.
     *
    ...
    private static final class TrophyCellsPlus2 implements Ability {
        ...
        @Override public void modify(RuleQuery q) {
            if (Resource.TROPHY.equals(q.subject())
                    || "trophy".equals(String.valueOf(q.subject()))) {
                q.add(2).by(id());
            }
        }
     ...
     * на место уничтоженных жетонов. ЦУ обрабатывается тем же путём, что и в обычном бою
