...
public final class PrintedContainers {
    ...
    public static boolean visibleContainer(GameState s, Hex h) {
        ...
        if (h.containerCell == BlockStamp.AIR) {
            // ВОЗДУШНАЯ ЯЧЕЙКА зданиями не накрывается, её занимает авиация:
            // контейнер в небе виден, пока в небо никто не встал.
            //
            // ДОБЫТЧИКУ ОН ВСЁ РАВНО НЕ ДОСТАЁТСЯ (правило дизайнера
            // 16.09.2026: «И НЕ ВОЗДУШНОМ») — но отсекается это не здесь, а в
            // minableContainerHex: там сектор контейнера обязан примыкать к сектору
            // добытчика, а воздушная ячейка (6) не примыкает ни к одному из шести.
            // Здесь же предикат общий: его читает и постановка жетонов, а авиация
            // воздушный контейнер берёт.
    ...
    /**
     * Гекс с ВИДИМЫМ контейнером, до которого добытчик ДОТЯГИВАЕТСЯ.
     *
     * <p><b>Правило (дизайнер, 16.09.2026, дословно).</b> «Добытчик добывает
     * контейнер, если тот напечатан на СОСЕДНЕМ С ДОБЫТЧИКОМ СЕКТОРЕ. И не
     * воздушном. И не просто на гексе. Плюс он может быть на соседнем гексе».
     *
     * <p>Отсюда ровно два случая:
     * <ul>
     *   <li><b>свой гекс</b> — контейнер на секторе, СОСЕДНЕМ по кругу с
     *       сектором добытчика (добытчик занимает один сектор, соседних у него
     *       два);</li>
     *   <li><b>соседний гекс</b> — добытчик стоит своей стенкой к этому гексу,
     *       и контейнер напечатан на том секторе соседа, который смотрит на
     *       добытчика через общую сторону.</li>
     * </ul>
     *
     * <p>ЧТО БЫЛО НЕ ТАК. Прежде на своём гексе брался контейнер с ЛЮБОГО
     * сектора — добытчик дотягивался через весь гекс; а на соседнем годился
     * любой сектор, даже дальний. Плюс годилась воздушная ячейка. Всё три —
     * расхождение с правилом, пойманное дизайнером 16.09.2026.
     *
     * @return гекс с добываемым контейнером или {@code null}, если брать нечего
     */
    public static String minableContainerHex(GameState s, BuildingToken miner) {
        ...
        for (int side = 0; side < 6; side++) {
            if (self.sideOwner[side] == null || self.sideOwner[side] != miner.uid) {
                continue;               // этим сектором добытчик не стоит
            }
            // СВОЙ ГЕКС: сектор контейнера — сосед по кругу с сектором добытчика.
            if (visibleContainer(s, self)
                    && (self.containerCell == (side + 1) % 6
                        || self.containerCell == (side + 5) % 6)) {
                return miner.hexId;
            }
            // СОСЕДНИЙ ГЕКС: контейнер на секторе, смотрящем на добытчика.
            String nbId = self.neighborBySide[side];
            Hex nb = nbId == null ? null : s.field.get(nbId);
            if (nb != null && visibleContainer(s, nb)
                    && nb.containerCell == (side + 3) % 6) {
                return nbId;
            }
        }
        return null;
