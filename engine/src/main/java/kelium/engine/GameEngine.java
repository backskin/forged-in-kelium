...
public final class GameEngine {
     ...
     * трофей, утилем карты задания. Нарисованный род — это МЕСТО, ОТКУДА он
    ...
    private void arsenalInstall(PlayerState p, String cid, Agent agent) {
        ...
        if (p.arsenalInstalled.size() >= 3) {
            ...
            // «+2 ячейки под трофеи»). Со снятием ячейки закрываются, и то, что
    ...
    private void returnStep(boolean gameEnding) {
        ...
        // «в Возврат ВСЕ трофеи конвертируются в трофеи» ниже: раньше это был
        ...
        if (!gameEnding && rs.getBool("return_step.return_destroyed_tokens", true)) {
            ...
            for (PlayerState p : s.players) {
                ...
                // «трофеи в трофеи» следующего игрока). Запись показывала
     ...
     * Отказаться тоже можно: задержанный трофей не конвертируется в трофей,
    ...
    private static Map<String, Object> resourcesMap(PlayerState p) {
        ...
        m.put("debris", p.resources.trophy());
        return m;
    }
