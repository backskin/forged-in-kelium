...
public final class FieldGeometry {
    ...
    //
    // ПОРЯДОК ГНЁЗД — ПЕЧАТНЫЕ ЦВЕТА ИГРЫ: голубой, алый, зелёный, жёлтый. Тот
    // же порядок у колод приказов в данных (blue, scarlet, green, yellow) и у
    // подписей в проигрывателе (Волк, Ястреб, Барс, Лиса), и по нему же
    // разложены присланные дизайнером рисунки жетонов (token/*_p1…_p4).
    //
    // ПРИВЕДЕНО К ПЕЧАТНЫМ 08.09.2026: второе гнездо было оранжевым, четвёртое
    // пурпурным — таких цветов в коробке нет. Пока жетоны рисовались силуэтами,
    // это было незаметно; с настоящими картинками силуэт-подложка спорила бы с
    // рисунком по цвету. Оттенки взяты те же, что у образцов колод
    // ({@code Names.orderDeckColour}), чтобы цвет игрока был один во всей игре.
    /** Бледная заливка (подложка сектора, панели). */
    public static final String[] SEAT_FILL = {"#cfe8ff", "#ffd8d3", "#d8f0d0", "#fdf0c8"};
    /** Обводка жетонов и рамок игрока. */
    public static final String[] SEAT_STROKE = {"#2b6cb0", "#a32c20", "#2f855a", "#9a7b12"};
    /** Насыщенный цвет игрока — заливка САМОЙ формы жетона. */
    public static final String[] SEAT_TOKEN = {"#3b82d0", "#c0392b", "#3f9e60", "#d1a62a"};
    ...
    public static double unitWidth(String type, int span, double size) {
        // РАЗМЕРЫ ЖЕТОНОВ ВОЙСК — ПО СТОЛУ, А НЕ ПО СХЕМЕ (заказ дизайнера
        // 08.09.2026: «очень маленькие жетоны техники… невероятно маленькие
        // жетоны вышек, они должны быть по размеру как добытчики с
        // энергостанциями, меньше на 5-10%»).
        //
        // Мера взята от ЖЕТОНА ЗДАНИЯ НА ОДНУ СТОРОНУ (добытчик, энергостанция):
        // он занимает 0,88 радиуса гекса — столько же, сколько сторона гекса. На
        // столе войско и добытчик — картонки сравнимого размера, а на поле
        // войска были в два-три раза мельче: пехота 0,36, вышка 0,46. Отсюда
        // новые доли, все привязаны к тем же 0,88:
        //   вышка   0,80 — добытчик минус 9 %, как и просил дизайнер;
        //   техника 0,78 — жетон встаёт в напечатанную ячейку завода целиком
        //                  (перекрывая название — так и задумано);
        //   пехота  0,52 и авиация 0,52 — они и на столе меньше;
        // Жетон шире своего сектора перекрывает соседние: так и на столе, где
        // картонки лежат друг на друге, если их на гексе много.
        double k = switch (type == null ? "" : type) {
            // ШИРИНА — ВДОЛЬ КРОМКИ, и она не равна высоте: печатные жетоны
            // техники и вышки вытянутые (1,37 и 1,54 к одному), и в сектор их
            // сажает не ширина, а высота. Поэтому радиус посадки считается по
            // ВЫСОТЕ жетона (unitSeatRadius(size, height)), а ширина остаётся
            // такой, какой её просил дизайнер: крупной, со столом наравне.
            //
            // Прежде и радиус, и ширина считались как у квадрата, из-за чего
            // техника вылезала за кромку гекса и наезжала на соседей
            // (дизайнер 09.09.2026: «с поворотами и размещением техники и вышек
            // полнейший пиздец»).
            case "vehicle" -> 0.70;
            case "tower" -> 0.62;
            case "aircraft" -> 0.46;
            // ПЕХОТА — САМЫЙ МЕЛКИЙ ЖЕТОН, и такой она и должна быть: дизайнер
            // на 0,44 сказал «нереально огромный» (08.09.2026). Держим 0,40 —
            // чуть крупнее прежних 0,36, но заметно мельче техники и вышки.
            // ПОБОЧНОЕ СЛЕДСТВИЕ: напечатанная на нейтрале ячейка под пехоту
            // остаётся крупнее жетона примерно в 1,45 раза (просили 1,2). Свести
            // ближе можно только раздув пехоту — а её как раз и просили унять.
            default -> span >= 2 ? 0.46 : 0.40;
        };
        return size * k;
    }

    /** Насколько далеко от центра лежит ВНЕШНИЙ край жетона войска. */
    private static final double UNIT_RIM = 0.84;

    /**
     * Радиус посадки жетона войска (центр картинки от центра гекса). Войска, как
     * и здания, прижаты к кромке гекса: между соседними жетонами остаётся щель,
     * а поля по внешнему краю почти нет (просьба дизайнера 13.08.2026).
     */
    ...
    /**
     * РАДИУС ПОСАДКИ ПОД КОНКРЕТНУЮ ВЫСОТУ ЖЕТОНА.
     *
     * <p>Один общий радиус для всех родов не годится: жетон лежит вдоль кромки,
     * и наружу он вылезает СВОЕЙ ВЫСОТОЙ, а не шириной. Считаем от кромки
     * внутрь — внешний край ложится на {@link #UNIT_RIM} радиуса при любом
     * роде, и жетон гарантированно остаётся в своём гексе.
     *
     * @param height высота жетона в тех же единицах, что и {@code size}
     */
    public static double unitSeatRadius(double size, double height) {
        return Math.max(size * 0.18, size * UNIT_RIM - height / 2);
    }
