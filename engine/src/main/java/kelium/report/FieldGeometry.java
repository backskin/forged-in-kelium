...
public final class FieldGeometry {
    ...
    public static double unitWidth(String type, int span, double size) {
        double k = switch (type == null ? "" : type) {
            ...
            case "tower" -> 0.46;
            case "aircraft" -> 0.42;
            // ПЕХОТА ПОДРОСЛА 0,36 → 0,44 (заказ дизайнера 08.09.2026). Мера не
            // из головы: на нейтральной постройке НАПЕЧАТАНА ячейка под пехоту, и
            // жетон обязан быть ей под стать. Ячейка была крупнее жетона в 1,55
            // раза; дизайнер попросил свести разницу к 1,2 — половину пути прошла
            // пехота (×1,22), половину сама постройка (см. paintNeutral).
            // Больше нельзя: ячейка сектора на радиусе посадки 0,60 радиуса, и
            // жетон шире 0,50 в неё уже не встанет.
            default -> span >= 2 ? 0.50 : 0.44;
        };
        return size * k;
    }
