...
public final class Locations {
     ...
     *
     * <p>С 18.09.2026 это {@code scenarios/NEW}, а не корень {@code scenarios}.
     * Дизайнер перерисовал все поля после онлайн-теста (между ЦУ три гекса
     * вместо четырёх, стартовые зарождения ближе к центру, больше обходных
     * путей), сложил свежие в {@code new}, а прежние убрал в {@code old} —
     * и сказал: «возьми НОВЫЕ поля».
     *
     * <p>Почему важно, что меряем и учим именно здесь. Геометрия меняет игру
     * сильнее любых боевых правил: разброс уничтоженных жетонов между полями
     * достигал 140%. Пока библиотека смотрела в корень, в набор попадали старые
     * поля вперемешку с новыми папками, и замер шёл по геометрии, в которую
    ...
    public static Path builtinLayoutFolder() {
        ...
        // ПО УМОЛЧАНИЮ — СТАРЫЕ, ПРОСТОРНЫЕ ПОЛЯ (решение дизайнера 20.09.2026).
        // Новые поля сводили ЦУ ближе, чтобы война шла чаще; это работало, но
        // ценой тесноты, и вместе с базовыми скоростями 2·2·3 нужды в ней
        // больше нет: на просторных полях ускорение дало сносов 2,19 -> 2,93,
        // а на тесных — ничего.
        return корень.resolve("old");
    }
