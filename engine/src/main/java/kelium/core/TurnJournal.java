...
public final class TurnJournal {
    ...
    public static final class TurnFacts {
        ...
        // === факты колоды 1.16.0 (таблица дизайнера 04-06.09.2026) ===
        /**
         * ЧТО СНЕСЕНО НА КАЖДОМ ГЕКСЕ за этот ход: гекс -> сколько чужих жетонов
         * там уничтожено. Нужно заданию «вычисти гекс целиком»: его нельзя
         * проверить ни общим счётом убийств, ни расстановкой после боя — на
         * пустом гексе не написано, было ли на нём что-то до залпа.
         */
        public final Map<String, Integer> destroyedOnHex = new HashMap<>();
        /**
         * УРОВНИ СНЕСЁННЫХ ЧУЖИХ ЭНЕРГОСТАНЦИЙ. Задание платит за снос ИМЕННО
         * крупной (3-4 уровня), а {@link #destroyedTypes} знает только род
         * здания.
         */
        public final Set<Integer> destroyedPlantLevels = new HashSet<>();
        /**
         * Места, чьи жетоны снесены за ход, вместе с числом ВОЙСК, которое было
         * у них на поле в момент удара: задание «бей того, у кого войск больше
         * твоего» обязано сравнивать положение НА МОМЕНТ УДАРА, а не на момент
         * проверки — к ней и войск уже нет.
         */
        public final Map<Integer, Integer> victimUnitsAtHit = new HashMap<>();
        /** Сколько СВОИХ войск было у бьющего на поле в момент удара. */
        public final Map<Integer, Integer> myUnitsAtHit = new HashMap<>();
        /**
         * СКОЛЬКО КОНТЕЙНЕРОВ ВЗЯТО ЗА ХОД — добытчиками и войсками вместе.
         *
         * <p>Прежде считались только пойманные ВОЙСКАМИ
         * ({@link #containersPickedByUnit}), а взятые добытчиком отмечались
         * одним признаком «да/нет»: задание «забери три контейнера» посчитать
         * было нечем.
         */
        public int containersTaken = 0;
        /**
         * СНЕСЁН ПОЛНОСТЬЮ ЗАПИТАННЫЙ ЧУЖОЙ ДОБЫТЧИК, СТОЯВШИЙ У КЕЛЕМИЯ.
         *
         * <p>{@link #destroyedPoweredEconomy} говорит только «здание было
         * запитано»; усиление карты просит большего — добытчик работал в полную
         * силу И ему было что добывать. После сноса этого уже не спросить: и
         * кубики, и соседство остались в прошлом ходе.
         */
        public boolean destroyedFullMinerAtKelium = false;
        /**
         * СДАНЫ В ЖЕРТВУ ДВА ЖЕТОНА ТЕХНИКИ И/ИЛИ АВИАЦИИ С ОДНОГО ГЕКСА.
         *
         * <p>Усиление карты «Разоружение» спрашивает, ЧТО именно сдано, а к
         * моменту проверки сданного на поле уже нет: плата снимается раньше. То,
         * что нельзя увидеть на столе после платежа, записывается в момент
         * платежа.
         */
        public boolean sacrificedStrikeGroup = false;
        ...
        /**
         * РАЗНЫЕ ОПЛАЧЕННЫЕ ПРЕДЛОЖЕНИЯ планшета рынка за ход — o33 «Биржа».
         ...
         * scienceTrophiesSpent и считало, насколько уменьшилось МЕСТО УНИЧТОЖЕННЫХ ЖЕТОНОВ.
         * Пока наука брала целые жетоны, это работало. Потом правило поменялось
         * (tech.pay_with_trophy_only: наука платится трофеями), место уничтоженных жетонов
        ...
        void reset() {
            ...
            // его видели все, а сбитый жетон лежит в твоём месте уничтоженных жетонов.
