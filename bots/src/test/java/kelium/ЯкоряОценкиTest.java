 ...
 * считается: келемий (0 ПО) стоял в оценке дороже обломка (0.5 ПО). Поймано это
...
class ЯкоряОценкиTest {
    ...
    void курсыСводаНеИзменились() {
        ...
        assertEquals(0.5,
            ((Number) rs.get("economy.debris_storage_vp_per_unit", 0)).doubleValue(), 1e-9,
            "ИЗМЕНИЛСЯ курс обломка — пересчитай eval.trophy_pool");
    }
    ...
    void считаемоеВышеНесчитаемого() {
        Genome g = Genome.defaults();
        // ГЛАВНАЯ ПРОВЕРКА. Обломок даёт пол-очка сам по себе, келемий — ноль;
        // значит обломок обязан стоить дороже. Именно это соотношение и было
        // перевёрнуто (0.7 против 0.9), и именно оно уводило ботов от войны.
        double обломок = g.get("eval.trophy_pool", 0);
        double келемий = g.get("eval.kelium", 0);
        org.junit.jupiter.api.Assertions.assertTrue(обломок > келемий,
            "обломок (0.5 ПО) должен цениться выше келемия (0 ПО), а сейчас "
            + обломок + " против " + келемий);
