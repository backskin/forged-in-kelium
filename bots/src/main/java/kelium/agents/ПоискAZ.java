...
public final class ПоискAZ extends Agent {
    ...
    private static final class Узел {
        ...
        /** Одновременная фаза, начатая в этом узле: узлы выбора остальных игроков. */
        final Map<Integer, Узел> одновременные = new HashMap<>();
        /** Узлы после одновременной фазы — по сочетанию выборов всех игроков. */
        final Map<String, Узел> послеФазы = new HashMap<>();
    ...
    /**
     * ОДНОВРЕМЕННЫЕ РЕШЕНИЯ: вскрытие приказов и слепой сброс. Движок спрашивает
     * игроков по очереди, но карту соседа никто не видит. В дереве поэтому
     * выбор каждого игрока в такой фазе НЕ зависит от выбора остальных: у
     * каждого свой узел, общий для любых чужих выборов, а партия дальше идёт по
     * узлу сочетания всех выборов.
     */
    private static boolean одновременно(String вид) {
        return "reveal_order".equals(вид) || "blind_discard".equals(вид);
    }
    ...
    private void спуск(Позиция корень, Узел дерево, int мест, Random r,
        ...
        // одновременная фаза: якорь, её вид и выборы игроков
        Узел[] якорь = {null};
        String[] видФазы = {null};
        java.util.TreeMap<Integer, String> выборыФазы = new java.util.TreeMap<>();
        ...
        for (int i = 0; i < мест; i++) {
            ...
            агенты.add(new Agent(i, "спуск#" + i) {
                ...
                public Choice choose(GameState st, List<Choice> options, Map<String, Object> ctx) {
                    ...
                    String вид = вид(ctx);
                    // конец одновременной фазы: дальше — узел сочетания выборов
                    if (видФазы[0] != null && !видФазы[0].equals(вид)) {
                        Узел я = якорь[0];
                        String сочетание = выборыФазы.toString();
                        Узел после = я.послеФазы.get(сочетание);
                        if (после == null) {
                            после = new Узел(место);
                            я.послеФазы.put(сочетание, после);
                        }
                        текущий[0] = после;
                        ждём[0] = null;
                        видФазы[0] = null;
                    ...
                    if (одновременно(вид)) {
                        ...
                        Узел я = якорь[0];
                        у = я.место == место ? я : я.одновременные.computeIfAbsent(место, Узел::new);
                    } else {
                        у = текущий[0];
                        if (у == null) {
                            у = new Узел(место);
                    ...
                    рёбра.add(лучшее);
                    if (видФазы[0] != null) {
                        выборыФазы.put(место, ключ(лучший));   // в фазе узел не сдвигается
                    } else {
