...
public class PlannerAgent extends Agent {
    ...
    private double оценить(GameState после, long seed) {
        ...
        double доиграно = Lookahead.playOut(после, seat, genome, others, null,
            horizonRounds, seed);
        if (Double.isNaN(доиграно)) {
            return статика;
        }
        // ПОЛОВИНА НА ПОЛОВИНУ. Чистое доигрывание шумнее статики: оно зависит от
        // случайных добров и от того, как модель соперника сыграет ответ. Смесь
        // сохраняет устойчивость статики и добавляет то, чего в ней нет вовсе, —
        // цену ответного хода. Вес подбирается замером, а не на глаз.
        return 0.5 * статика + 0.5 * доиграно;
    }
