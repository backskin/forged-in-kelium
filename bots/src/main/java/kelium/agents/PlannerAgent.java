...
public class PlannerAgent extends Agent {
    ...
    public static PlannerAgent ofLevel(int level, String character, int seat, Random rng,
        ...
        return switch (Math.max(2, Math.min(4, level))) {
            ...
            default -> сГоризонтом(
                new PlannerAgent(seat, rng, g, character, 10, true, players));
        };
    }

    /**
     * ГРОССМЕЙСТЕР СМОТРИТ НА ОТВЕТ СОПЕРНИКА (включено по умолчанию 19.09.2026).
     *
     * <p>Уровень умения в этой игре — это глубина мысли, и горизонт ровно она и
     * есть: бот проигрывает не только свой ход, но и ответный раунд, и оценивает
     * то, что осталось ПОСЛЕ ответа. Мастеру и любителю горизонт не даётся
     * намеренно — иначе уровни перестанут отличаться, да и считают они дешевле.
     *
     * <p>Настройка выбрана вилкой на 150 парных раздачах (punisher, поля new):
     * <pre>
     *   без горизонта                        14.0% побед
     *   горизонт 1, вес 0.50, одно доигрывание  20.7%,  отрыв +1.49 ПО ± 0.91
     *   горизонт 1, вес 0.75, одно доигрывание  17.3%,  +0.81 — в шум
     *   горизонт 1, вес 0.50, три доигрывания   20.7%,  +1.46 — то же втридорога
     *   горизонт 1, вес 1.00, три доигрывания   20.0%,  +1.05
     * </pre>
     *
     * <p>Отсюда и значения: смесь ПОПОЛАМ со статической оценкой (больше веса
     * доигрыванию — хуже, оно шумное) и ОДНО доигрывание (усреднение трёх не
     * добавило ничего, а стоит втрое). Горизонт в два раунда проверялся раньше
     * и оказался хуже одного: вторая волна догадок о чужом ходе вносит больше
     * шума, чем сведений.
     *
     * <p>Это единственная правка ботов за 19.09.2026, давшая измеримую прибавку:
     * два прогона обучения весов (с поведенческой целью и с чистой) не дали
     * ничего — см. разбор, разделы 21-22.
     */
    private static PlannerAgent сГоризонтом(PlannerAgent бот) {
        бот.horizonRounds = 1;
        бот.horizonWeight = 0.5;
        бот.horizonSamples = 1;
        return бот;
    }
    ...
    /**
     * СКОЛЬКО РАУНДОВ ВПЕРЁД ДОИГРЫВАТЬ при оценке хода. 0 — как было: позиция
     * оценивается сразу после моего хода и ответ соперников не спрашивается.
     *
     * <p>Почему это главное место во всём боте. Планировщик перебирает порядки
     * СВОИХ действий и берёт тот, после которого позиция выглядит лучше всего.
     * Но «выглядит лучше» считается формулой в тот миг, когда соперники ещё не
     * ходили: бот не отличает выигрышную позицию от той, что рассыплется в
     * ответном бою. Никакой подбор весов этого не лечит — не хватает не весов,
     * а горизонта. Доигрывание ({@link Lookahead#playOut}) в движке было давно,
     * но планировщик не звал его ни разу; им пользовалась только старая линия
     * {@link SearchAgent}.
     *
     * <p>Цена — линейная по горизонту: каждый лишний раунд это ещё одна копия
     * стола и ещё один раунд движка на КАЖДЫЙ рассматриваемый сценарий.
    ...
    /**
     * СКОЛЬКО ДОИГРЫВАНИЙ УСРЕДНЯТЬ. Одно доигрывание — это один случайный
     * исход: свои доборы, свой ответ соперника. Замер 19.09.2026 показал,
     * во что это обходится: горизонт в ДВА раунда оказался ХУЖЕ одного
     * (19.5% побед против 22.0%, значимость пропала), потому что второй раунд
     * грубой модели добавляет больше шума, чем сведений. Значит лечить надо
     * не глубину, а разброс: среднее из нескольких прогонов.
     */
    public int horizonSamples = 1;

    /**
     * ДОЛЯ ДОИГРЫВАНИЯ В ОЦЕНКЕ (0 — только статика, 1 — только доигрывание).
     * Статика устойчива, но слепа к ответу соперника; доигрывание зряче, но
     * шумно. Половина на половину — отправная точка, а не истина: вес
     * подбирается замером.
     */
    public double horizonWeight = 0.5;

    /**
     * ЧЕГО СТОИТ ПОЗИЦИЯ ПОСЛЕ МОЕГО ХОДА. Без горизонта — статическая оценка,
    ...
    private double оценить(GameState после, long seed, double статика) {
        ...
        return (1 - horizonWeight) * статика + horizonWeight * (сумма / удачных);
    }
