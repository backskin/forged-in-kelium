...
public final class Fitness {
    ...
    public enum Goal {
         ...
         * жетоны, и ценность уничтожения меряется их ТРОФЕЙНОЙ ЦЕНОЙ (в трофеях),
    ...
    public static Result play(int players, long seed, int seat, Genome cand,
            ...
            switch (type) {
                case "combat_hit" -> {
                    ...
                    if (Boolean.TRUE.equals(ev.get("destroyed"))) {
                        ...
                        // ТРОФЕЙНАЯ ЦЕНА уничтоженного (в трофеях) — нужна цели
        ...
        if (goal == Goal.АКСИОМА) {
            ...
            // (2) УНИЧТОЖЕНИЕ ПО ТРОФЕЙНОЙ ЦЕНЕ (в трофеях), не «штука за
