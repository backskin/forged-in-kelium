...
public final class Fitness {
    ...
    public enum Goal {
         ...
         * жетоны, и ценность уничтожения меряется их ТРОФЕЙНОЙ ЦЕНОЙ (среди уничтоженных жетонов),
    ...
    public static Result play(int players, long seed, int seat, Genome cand,
            ...
            switch (type) {
                case "combat_hit" -> {
                    ...
                    if (Boolean.TRUE.equals(ev.get("destroyed"))) {
                        ...
                        // ТРОФЕЙНАЯ ЦЕНА уничтоженного (среди уничтоженных жетонов) — нужна цели
        ...
        if (goal == Goal.АКСИОМА) {
            ...
            // (2) УНИЧТОЖЕНИЕ ПО ТРОФЕЙНОЙ ЦЕНЕ (среди уничтоженных жетонов), не «штука за
        ...
        if (goal == Goal.ЗАДАЧНИК) {
            ...
            //
            // ВЕС ПОДНЯТ 18.08.2026 (замер designer'а: линия квестер выполняла
            // 1.27 задания за партию — не лучше среднего 1.28 по всему столу).
            // Причина: при базовом весе (3.0/2.0) бонус за задания был мал
            // относительно веса победы (60.0) и margin — отбор не видел
        ...
        if (goal == Goal.СУПЕРОРУЖИЕ) {
            ...
            // Мерить тут можно ровно одно — сколько низов уже отработало
            // (супер-заданий у игрока может быть несколько).
            fitness += 30.0 * s.player(seat).superObjectivesDone.size();
        }
        ...
        if (goal == Goal.ОХОТНИК) {
            ...
            //
            // ВЕС ПОДНЯТ 18.08.2026 (замер designer'а: военная победа во всём
            // столе — 4.67% партий, цель 10%). Тот же класс проблемы, что у
            // ЗАДАЧНИКА: бонус тонул на фоне веса обычной победы (60.0).
            fitness += 3.0 * c.getOrDefault("cu_hits", 0)
                + 20.0 * s.player(seat).cuDestructionTokens;
        }
