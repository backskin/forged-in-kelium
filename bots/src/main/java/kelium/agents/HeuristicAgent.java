...
public class HeuristicAgent extends Agent {
    ...
    protected java.util.function.BiFunction<GameState, Choice, Double> scorerFor(String kind, Map<String, Object> ctx) {
        return switch (kind) {
            ...
            case "red_slot" -> (s, o) -> {
                ...
                return глухой
                    ? 3.0 - 4.0 * п - 0.5 * своих - 1.5 * весРода(куда)
                    : 1.0 + 4.0 * п + 0.5 * своих + 1.5 * весРода(куда);
            };
    ...
    private double scoreModuleRed(GameState state, Choice o) {
        ...
        // ЧЕГО СТОИТ САМ РОД (19.09.2026): модуль на технику и авиацию ценнее,
        // чем на пехоту и тем более на вышку, — он усиливает жетон, который и
        // без него сильнее. См. весРода: одна шкала с оценкой позиции.
        v += 1.5 * весРода(ut);
        return v;
    }
