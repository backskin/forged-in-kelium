...
public final class ВоеннаяЭкономика {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int g = 0; g < партий; g++) {
            ...
            long[] счёт = new long[8];   // попаданий, сносов, боёв, холостых, залпов,
                                         // мог-но-не-выстрелил, некого-бить
                    ...
                    case "action" -> {
                        ...
                        if ("combat".equals(имя)) {
                            ...
                            if (чис(м.get("battle")) <= 0) {
                                ...
                                // ПОЧЕМУ ХОЛОСТОЙ. Движок сам называет причину:
                                // could_fight = было кому и по кому стрелять, но
                                // залп не состоялся (не хватило боеприпасов или
                                // бот отказался), иначе — стрелять было некого.
                                if (Boolean.TRUE.equals(м.get("could_fight"))) {
                                    счёт[5]++;           // мог, но не выстрелил
                                } else if (Boolean.TRUE.equals(м.get("targets_in_reach"))) {
                                    счёт[6]++;           // цель была, боеприпаса нет
                                } else {
                                    счёт[7]++;           // цели в досягаемости нет
                                }
                            }
        ...
        out.printf("    из холостых: цели в досягаемости НЕТ %.0f%%, "
                + "цель была, но НЕЧЕМ ПЛАТИТЬ %.0f%%, мог и не стал %.0f%%%n",
            холостых == 0 ? 0 : 100 * нетЦели / холостых,
            холостых == 0 ? 0 : 100 * нечемПлатить / холостых,
            холостых == 0 ? 0 : 100 * могНоНе / холостых);
    ...
    private static boolean зданиеЛи(String подпись) {
        // Подпись жертвы приходит из CombatResolver.victimLabel: у зданий это код
        // типа плюс уровень («power_plantL2»), у войск — просто род. Родов ровно
        // четыре, всё остальное — здания. Коды берутся ПОЛНЫЕ («infantry», а не
        // «inf»): на сокращениях предикат молча считал зданиями всё подряд и
        // выдавал «зданий уничтожено 100% всех потерь».
        String к = подпись == null ? "" : подпись.trim();
        return !(к.equals("infantry") || к.equals("vehicle")
            || к.equals("aircraft") || к.equals("tower"));
    }
