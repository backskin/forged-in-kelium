...
(Scenario: обычный тайл 4 на лице и 3 на обороте, стартовый — 3 и 2):
  ...
  2 → spawn_start           лицо стартового (надпись Start), 3
  3 → spawn_flipped         оборот обычного, 3
  4 → spawn_start_flipped   оборот стартового (Start), 2

Порядок экспорта 24.09.2026 прочитан по самим картинкам: стартовый тайл идёт
вторым. Пятый файл экспорта — голый гекс поля, он не тайл и сюда не идёт.
Запретные гексы в экспорте 21.09 лежат только листом для TTS
(«запретные гексы.png», 3×3 на чёрном фоне) — он без прозрачности, и резать его
нельзя: углы гексов выйдут чёрными. Пока отдельных файлов нет, запретные не
трогаются, в игре остаются прежние девять.
...
ЗАРОЖДЕНИЕ = [
    ...
    ('Жетон гекса зарождения-2.png', 'spawn_start'),
    ('Жетон гекса зарождения-3.png', 'spawn_flipped'),
...
def main():
    ...
    запретные = sorted(
        ...
        if f.lower().endswith('.png') and not f.startswith('запретные гексы'))
    if not запретные:
        print('отдельных запретных гексов в экспорте нет — прежние не трогаю')
