...
def storage_stores(path):
    ...
    боксы = []
    # ТРЕТЬЕ МЕСТО В СЕРЕДИНЕ (экспорт 25.09.2026): на новых планшетах между
    # двумя складами напечатан третий, а маленькие ячейки начала игры ушли
    # вниз. Поэтому середина больше не отбрасывается целиком: мелочь (ячейки,
    # плашка цены) и так не проходит по размеру.
    for c in components(полоса, lambda r, g, b: r > 195 and g > 190 and b > 185, 5, 4000):
        if c[2] < w * 0.10 or c[3] < h * 0.16:
            continue
        if c[2] > w * 0.20 or c[3] > h * 0.35:
            continue                    # светлая подложка всей середины (storage-A)
        боксы.append((c[0] + dx, c[1] + dy, c[2], c[3]))
...
def места_хранилища(board_id, path, problems):
    ...
    # два места — прежняя печать, три — экспорт 25.09.2026
    if len(боксы) not in (2, 3):
        problems.append('%s: мест под жетоны хранилища нашлось %d — ждали 2 или 3'
                        % (board_id, len(боксы)))
        return []
