...
# номер карты в экспорте, имя игрока, что он разыгрывает и какие кружки
# на карте обвести. Ячейки: 0 и 1 — действия выбранного приказа (верх),
# 2 и 3 — действия чужого приказа (низ).
КРУГ = [
    (1, "Аксель", "Стройка · Смена энергии", (0, 1)),
    (7, "Бриана", "Рынок · Наука", (0, 1)),
    (12, "Вэнс", "Манёвр · Бой · Стройка", (0, 1, 2)),
    (15, "Глория", "Наука · Смена энергии", (1, 3)),
]

# Где на карте сидят четыре значка действий — в долях от её ширины и высоты.
ЯЧЕЙКИ = [(0.268, 0.314), (0.675, 0.314), (0.268, 0.748), (0.675, 0.748)]
...
for k, (номер, имя, дело, играет) in enumerate(КРУГ):
    x = k * (Ш + ЗАЗОР)
    ...
    # ЧТО ИГРОК РАЗЫГРЫВАЕТ, обведено прямо на карте: иначе пример читается
    # только по тексту под ним (просьба дизайнера 15.09.2026).
    for я in играет:
        дx, дy = ЯЧЕЙКИ[я]
...
холст.save(png)
figure("круг", png, 2000, W, H, [], css_w="100%", pad="1.2mm 0 5.4mm")
print("ok", (W, H))
