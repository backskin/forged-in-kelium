...
class Раскладчик:
    ...
    def _выложить(self):
        ...
        свободно = self.осталось - ШАПКА_ММ
    ...
    # ---------------------------------------------------------------- приём
    def раздел_начать(self, буква, имя, счёт):
        self._выложить()
        self.копилка = ((шапка(буква, имя, счёт), (буква, имя)), [])
...
def задания(книга, версии, р):
    ...
    for имя, набор_карт, буква in (("Карты заданий", обычные, БУКВА.next()),
                                   ("Начальные задания", начальные, БУКВА.next())):
        р.раздел_начать(буква, имя, "%d карт" % len(набор_карт))
        for c in набор_карт:
            строки = [
            ...
            р.карточка(карточка(c["name"], ВИД_ЗАДАНИЯ.get(c.get("type"), ""),
...
def арсенал(книга, версии, р):
    ...
    обычные = [c for c in из if c.get("kind") != "starting"]
    начальные = [c for c in из if c.get("kind") == "starting"]
    for имя, набор_карт, буква in (("Карты арсенала", обычные, БУКВА.next()),
                                   ("Начальный арсенал", начальные, БУКВА.next())):
        р.раздел_начать(буква, имя, "%d карт" % len(набор_карт))
        for c in набор_карт:
            строки = ['<span class="кл">верх</span>%s' % текст(c.get("top")),
                      '<span class="кл">низ</span>%s' % текст(c.get("bottom"))]
            р.карточка(карточка(c["name"], "", строки,
                                "нач" if буква == "Г" else ""),
                       высота_карточки(строки))


def контейнеры(книга, версии, р):
    из = данные.набор("containers", версии["containers"])
    р.раздел_начать(БУКВА.next(), "Карты контейнеров", "%d карт" % len(из))
    for c in из:
        строки = ['<span class="кл">даёт</span>%s' % текст(c.get("a"))]
        р.карточка(карточка(c["name"], ЯРУС.get(c.get("tier"), ""), строки),
                   высота_карточки(строки))
...
def рынок(книга, версии, р):
    ...
    for c in из:
        ...
        строки = ['<span class="кл">слева</span><span><b>%s.</b> %s</span>'
                  % (лев.get("name", "—"), текст(лев)),
                  '<span class="кл">справа</span><span><b>%s.</b> %s</span>'
                  % (прав.get("name", "—"), текст(прав))]
...
def супер(книга, версии, р):
    ...
    р.раздел_начать(БУКВА.next(), "Супер-задания", "%d карт" % len(зад))
    for c in зад:
        строки = ['<span class="кл">очки за</span>%s'
                  % ", ".join(c.get("categories", []))]
        р.карточка(карточка(c["name"], "супер-задание", строки),
                   высота_карточки(строки))
    р.раздел_начать(БУКВА.next(), "Супер-арсенал", "%d карт" % len(арс))
    for c in арс:
        строки = ['<span class="кл">на карте</span>%s'
                  % (c.get("текст_карты") or текст(c))]
        р.карточка(карточка(c["name"], c.get("kind", ""), строки),
                   высота_карточки(строки))
    ...
    for состав, ключ, сколько in РАСКЛАДКИ:
        ...
        if os.path.exists(легенда) and состав == РАСКЛАДКИ[-1][0]:
            низ = ('\n    <div class="легенда-поля"><img src="%s" alt=""></div>'
                   % b64(легенда, 1400))
        книга.стр(шапка(б, "Раскладки поля · " + состав,
