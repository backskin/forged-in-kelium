...
ЗАЗОР_ММ = 1.9              # промежуток между карточками
...
def контейнеры(книга, версии, р):
    ...
    р.раздел_начать("Д", "Карты контейнеров", "%d карт" % len(из))
    for c in из:
...
def рынок(книга, версии, р):
    ...
    р.раздел_начать("Е", "Карты рынка", "%d карт" % len(из))
    for c in из:
