...
def супер(книга, версии, р):
    ...
    р.раздел_начать(
        ...
        вступ="Супер-задание не выполняют и не сжигают: оно лежит у вас "
              "открытым до конца партии и там считается сразу по двум статьям. "
              "Статьи считаются отдельно одна от другой, поэтому решают не "
              "рывки, а то, с чем вы придёте к последнему ходу.",
        вступ_мм=10.0)
    for c in зад:
        части = [словарь_кат.get(к, к) for к in c.get("categories", [])]
        текст = "; ".join(части) + "."
