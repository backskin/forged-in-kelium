...
# НЕСЖАТАЯ КОПИЯ С ТЕКСТОВЫМ СЛОЕМ. Потоки страниц лежат в файле как есть,
# поэтому текст читается прямо в PDF и вычитывается любым инструментом; сами
# картинки остаются сжатыми — с распакованными файл пухнет до полугигабайта.
сырой = os.path.join(D, "book-raw.pdf")
doc.save(сырой, deflate=False, deflate_images=True, deflate_fonts=True,
         garbage=0, clean=False, pretty=True)
print("несжатая копия: %.1f МБ" % (os.path.getsize(сырой) / 1e6))
