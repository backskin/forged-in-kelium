...
<li><b>+ 1</b> {и('спец-действие')} <b>спец-действие:</b> карта задания или арсенала, вскрытие контейнеров, спец-плашка карты приказа — Монеты · Боеприпас · Задание · Движение.</li>
<li><b>«Затаиться»</b> — любые <b>2</b> разных действия из восьми и спец-действие. Совпадения не боится и не создаёт.</li>
<li>Действия — в любом порядке, одно за другим. Можно сыграть меньше.</li>
</ul>
"""

ДЕЙСТВИЯ = [
    ("манёвр", "Манёвр", "Разместить", f"""
...
<li>Стенки закрывают проход. Пехота может войти в гарнизон.</li>"""),
    ("стройка", "Стройка", "Разместить", f"""
<li><b>Поставить</b> здание из запаса за его цену {и('монета')} на свободные сектора подряд в зоне стройки.</li>
<li><b>Снести</b> своё здание в запас: +1 {и('монета')}. ЦУ — только если осталось спец-действие: им сразу ставите ЦУ заново.</li>
<li>С каждым зданием — одно из двух за действие.</li>"""),
    ("смена-энергии", "Питание", "Контролировать", f"""
<li>Переложите любые свои кубики {и('энергия')} куда хотите:</li>
<li>с источников на ячейки {и('ячейка-энергии')} своих зданий, с ячейки на ячейку</li>
<li>и обратно на источники.</li>"""),
    ("бой", "Бой", "Контролировать", f"""
...
<li>или <b>карту контейнера</b> {и('контейнер')} с соседнего сектора. Опустошил тайл — награда с оборота.</li>"""),
    ("наука", "Наука", "Исследовать", f"""
<li>Платите трофеями {и('иконка-трофея')} {и('трофей')} — кубиками или жетонами со свалки:</li>
<li><b>ступень трека</b> по порядку — 1 / 2 / 3 / 4, на каждом треке не больше одной;</li>
<li>каждый обмен один раз: <b>1</b> — смена модуля · <b>2</b> — позолота · <b>2</b> — карта арсенала.</li>"""),
    ("снаряжение", "Снабжение", "Приобрести", f"""
...
<li>или <b>боеприпасы</b> {и('боеприпас')} в хранилище. Числа — на планшете войск.</li>"""),
    ("рынок", "Рынок", "Приобрести", f"""
<li>Каждый обмен — один раз за действие: 0 / 1 / 2 {и('келемий')} → 1 / 3 / 7 {и('монета')} (одна ступень), 1 {и('келемий')} → 2 {и('карта-задания')}, одну оставьте.</li>
