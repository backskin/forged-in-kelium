...
def мед(name):
    return f'<img class="мед" src="{b64("д-" + name, 260)}" alt="">'
...
<li>Фишка {и('первый-игрок')} первого игрока — соседу по часовой стрелке.</li>
</ul>
...
<li><b>Трофеи:</b> каждый жетон на свалке с {и('шестерня')} на обороте — 1 кубик трофея в свободную ячейку хранилища. Жетоны — владельцам.</li>
...
<ul>
<li><b>Выбранный (верхний) приказ</b> — оба {и('приказ')} действия, каждое один раз. Такой же приказ уже вскрыт в этом круге — только <b>одно</b>.</li>
<li><b>Чужой (нижний) приказ</b> {и('чужой-приказ')} — одно действие, если его как выбранный уже вскрыл другой игрок.</li>
<li><b>+ 1</b> {и('спец')} <b>спец-действие:</b> карта задания или арсенала, вскрытие контейнеров, спец-плашка карты приказа — Движение · Монета · Задание.</li>
...
<li><b>жетон</b> {и('войска')} своего рода войск — внутрь здания или на его гекс,</li>
...
<li>Обмены — сколько угодно раз: 1 {и('келемий')} → 3 {и('монета')}, 2 {и('келемий')} → 7 {и('монета')}, 1 {и('келемий')} → 2 {и('карты-заданий')}.</li>
<li><b>Предложение карты рынка</b> — одно за действие; келемий ложится в ячейку карты.</li>"""),
    ("наука", "Наука", "Приобретения", f"""
<li>Платите трофеями {и('шестерня')} {и('куб-трофей')} — кубиками или жетонами со свалки:</li>
...
<li class="победа"><b>Военная победа</b> {и('уничтожен')} — уничтожить ЦУ, уже имея перевёрнутый модуль блокировки боя.</li>
</ul>
"""

ОЧКИ = f"""
<div class="шапка"><span class="таб">Победные очки</span><i>{и('звезда')} только это</i></div>
<ul class="две-кол">
...
ЛЕГЕНДА = [
    ...
    ("куб-трофей", "трофей"), ("энергия", "энергия"), ("звезда", "очко"),
    ("приказ", "действие"), ("спец", "спец-действие"), ("карта-задания", "задание"),
    ("контейнер", "контейнер"),
]
...
<div class="разворот одна">
  <div class="стр обложка задняя">
    ...
    <div class="пан" style="left:8mm;top:131mm;width:63mm;height:68.5mm"><div class="пан-в">{ПРИКАЗ}</div></div>

    <div class="пан дейпан" style="left:74mm;top:26mm;width:138mm;height:131mm"><div class="пан-в">
      <div class="шапка"><span class="таб">Восемь действий</span><i>по два на каждый приказ</i></div>
      <div class="дей-сетка">{сетка}</div>
    </div></div>

    <div class="пан низ" style="left:74mm;top:159.5mm;width:63mm;height:40mm"><div class="пан-в">{КОНЕЦ}</div></div>
    <div class="пан низ" style="left:140mm;top:159.5mm;width:72mm;height:40mm"><div class="пан-в">{ОЧКИ}</div></div>
