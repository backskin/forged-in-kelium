    ...
    private static java.util.List<Крыло> крылья(Rectangle лист) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int ч = 0; ч < частей; ч++) {
                ...
                if (частей == 2) {
                    // ДВА КРЫЛА НА ОДНОЙ КРОМКЕ СТОЯТ БЛИЖЕ К ЕЁ СЕРЕДИНЕ, чем
                    // ровные половины: у самого угла планшета печати нет, там
                    // сходятся кромки. Ровные половины отодвигали жетон к углу, и
                    // сбоку оставался зазор (замечание дизайнера 09.09.2026:
                    // «первый и четвёртый уровень чуть съехали от центра»).
                    // Сдвиг симметричный — оба крыла кромки едут к её середине.
                    double серx = (a[0] + b[0]) / 2;
                    double серy = (a[1] + b[1]) / 2;
                    double к = Math.hypot(серx - сx, серy - сy);
                    double шаг = длина * 0.065;
                    сx += (серx - сx) / к * шаг;
                    сy += (серy - сy) / к * шаг;
                }
