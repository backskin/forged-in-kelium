...
public final class CardWords {
    ...
    private static String ресурс(String ключ, int n) {
        return switch (ключ == null ? "" : ключ) {
            ...
            case "trophies" -> счёт(n, "трофейный жетон со своего трофейного места",
                "трофейных жетона со своего трофейного места",
                "трофейных жетонов со своего трофейного места");
    ...
    public static String условие(String предикат, Map<String, Object> п) {
        ...
        return switch (предикат) {
            ...
            // ---- трофеи ----
            case "trophy_contains" -> трофейноеМесто(п);
            case "trophy_distinct_kinds" -> "имей на трофейном месте жетоны "
            ...
            // ---- приказы ----
            case "assembly_all_chose_ammo" -> "проведи Сборку, в которой ВСЕ твои "
    ...
    private static String видыСборки(Map<String, Object> п) {
    ...
    /** Что должно лежать на трофейном месте. */
    private static String трофейноеМесто(Map<String, Object> п) {
        List<String> типы = список(п, "building_types");
        if (!типы.isEmpty()) {
            return "имей на трофейном месте чужое здание из числа: " + зданияСписком(типы);
        }
        if (п != null && п.containsKey("any")) {
            return "имей на трофейном месте " + жетонов(ч(п, "any", 1)) + " противника";
        }
        int n = ч(п, "building", 1);
        return "имей на трофейном месте " + счёт(n, "чужое здание", "чужих здания",
            "чужих зданий");
    }
